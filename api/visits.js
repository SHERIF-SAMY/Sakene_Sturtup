  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      await supabase.from('notifications').insert({
        ...
        body: `تم استلام طلب حجزك لـ ${propNum} "${propTitle}" بتاريخ ${visit_date} الفترة ${visit_time}. لإتمام الحجز، يرجى دفع 200 ج رسوم حجز عبر فودافون كاش أو إنستا باي على الرقم: 01068411434 وإرسال صورة الإيصال.`,
        type: 'new_visit',
        is_read: false,
      });
      ...
      if (ownerUserId) {
        ...
        await supabase.from('notifications').insert({
          ...
          title: '🏠 طلب حجز لشقتك',
          body: isBrokerAcc
            ? `يوجد طلب حجز جديد لشقتك ${propNum} "${propTitle}" بتاريخ ${visit_date}. في حالة تم الاتفاق الكامل مع المستأجر، هناك رسوم خدمة بقيمة 400 ج تُدفع عبر فودافون كاش أو إنستا باي على الرقم: 01068411434.`
            : `يوجد طلب حجز جديد لشقتك ${propNum} "${propTitle}" بتاريخ ${visit_date}. في حالة تأجير شقتك، هناك رسوم خدمة بقيمة 200 ج تُدفع عبر فودافون كاش أو إنستا باي على الرقم: 01068411434.`,
          type: 'new_visit',
          is_read: false,
        });
      }
